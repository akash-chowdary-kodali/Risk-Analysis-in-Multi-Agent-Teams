...
def parse_filename_for_agents_and_episode(filename):
    ...
    if match:
        agent1_raw = match.group(1).strip()
        agent2_raw = match.group(2).strip()
        
        # Normalize agent names: replace underscores with spaces
        agent1_name = agent1_raw.replace("_", " ")
        agent2_name = agent2_raw.replace("_", " ")
        
        episode_number = int(match.group(3))
        
        # Check normalized names against the canonical list
        if agent1_name in ALL_AGENT_NAMES and agent2_name in ALL_AGENT_NAMES:
            return agent1_name, agent2_name, episode_number # Return normalized names
    return None

def get_stats_and_distance_series_from_json_file(filepath, agent1_name_from_filename, agent2_name_from_filename):
    # agent1_name_from_filename and agent2_name_from_filename are now expected to be normalized (space-separated)
    try:
    ...
    return {
        ...
        "agent1_name_in_file": agent1_name_from_filename, # This will be the normalized name
        "agent2_name_in_file": agent2_name_from_filename, # This will be the normalized name
...
def analyze_and_plot_average_stats_and_distances(folder_path, episodes_to_average=range(1, 21)):
    ...
    print(f"--- Showing plots for agents vs {TARGET_AGENT} ---")

    for canonical_pair, episode_data_list in aggregated_pair_episode_data.items():
        # N1_name_sorted and N2_name_sorted are normalized and sorted
        N1_name_sorted, N2_name_sorted = canonical_pair 

        if TARGET_AGENT not in canonical_pair:
            continue

        num_episodes_for_this_pair = len(episode_data_list)
        title_agent_display1 = ""
        title_agent_display2 = TARGET_AGENT

        if N1_name_sorted == TARGET_AGENT:
            if N2_name_sorted == TARGET_AGENT: 
                title_agent_display1 = TARGET_AGENT
            else: 
                title_agent_display1 = N2_name_sorted
        else: 
            title_agent_display1 = N1_name_sorted

        print(f"\n=== Pair: {title_agent_display1} vs {title_agent_display2} (based on {num_episodes_for_this_pair} episodes) ===")
        ...
        all_individual_distances_for_pair = [] 
        ...
        for ep_data in episode_data_list:
            ...
            # current_file_agent1 is now the normalized name from ep_data
            current_file_agent1 = ep_data["agent1_name_in_file"] 
            # Comparison is now between consistently normalized names
            if current_file_agent1 == N1_name_sorted: 
            ...
            else: # current_file_agent1 must be N2_name_sorted
        ...
        # agent_name_in_pair_actual is a normalized name (N1_name_sorted or N2_name_sorted)
        for agent_name_in_pair_actual, agent_stats_aggr in [(N1_name_sorted, N1_stats_aggr), (N2_name_sorted, N2_stats_aggr)]:
            print(f"  Average Stats for {agent_name_in_pair_actual} (in this pair):")
        ...
        if not padded_series_list:
            print(f"  No distance series data to plot average for pair: {title_agent_display1} vs {title_agent_display2}.")
        else:
            try:
            ...
            if len(plot_values_valid) > 0:
                ...
                plt.title(f"Average Euclidean Distance (Episodes {min(episodes_to_average)}-{max(episodes_to_average)}, n={num_episodes_for_this_pair})\n{title_agent_display1} vs {title_agent_display2}", fontsize=14)
                plt.xlabel("Timestep", fontsize=12)
                plt.ylabel("Average Euclidean Distance", fontsize=12)
                plt.grid(True, linestyle='--', alpha=0.7)
                plt.legend()
                plt.tight_layout()
                print(f"  Displaying average distance plot for: {title_agent_display1} vs {title_agent_display2}")
                plt.show()
            else:
                print(f"  No valid averaged distance data to plot for pair: {title_agent_display1} vs {title_agent_display2}.")
        ...
        if all_individual_distances_for_pair:
            ...
            plt.figure(figsize=(13, 7))
            plt.hist(all_individual_distances_for_pair, bins=custom_bin_edges, color='darkcyan', edgecolor='black', alpha=0.7) 
            plt.title(f"Histogram of Euclidean Distances (Bin Width: {bin_width:.2f})\n(Episodes {min(episodes_to_average)}-{max(episodes_to_average)}, n={num_episodes_for_this_pair})\n{title_agent_display1} vs {title_agent_display2}", fontsize=14)
            plt.xlabel("Euclidean Distance", fontsize=12)
            plt.ylabel("Frequency (Count)", fontsize=12)
            plt.grid(True, linestyle='--', alpha=0.5)

            current_ax = plt.gca()
            if len(custom_bin_edges) > 40: 
                current_ax.xaxis.set_major_locator(plt.MaxNLocator(nbins=15, prune='both')) 
            else: 
                plt.xticks(custom_bin_edges)
            
            plt.xticks(rotation=45, ha="right")
            plt.tight_layout()
            print(f"  Displaying distance histogram with bin width {bin_width:.3f} for: {title_agent_display1} vs {title_agent_display2}") 
            plt.show()
        else:
            print(f"  No valid individual distance data to plot histogram for pair: {title_agent_display1} vs {title_agent_display2}.")
